...
export async function analyzeProfitLoss(
  ...
  try {
    ...
    const txsLastDay = transactions.filter(tx => 
      tx.blockTime && new Date(tx.blockTime).getTime() > now - day
    );
    
    const txsLast3Days = transactions.filter(tx => 
      tx.blockTime && new Date(tx.blockTime).getTime() > now - 3 * day
    );
    
    const txsLastWeek = transactions.filter(tx => 
      tx.blockTime && new Date(tx.blockTime).getTime() > now - 7 * day
    );
    
    const txsLastMonth = transactions.filter(tx => 
      tx.blockTime && new Date(tx.blockTime).getTime() > now - 30 * day
    );
    
    const txsLast6Months = transactions.filter(tx => 
      tx.blockTime && new Date(tx.blockTime).getTime() > now - 180 * day
    );
...
export async function getTokenPurchases(
  ...
  try {
    ...
    for (const mint of Object.keys(knownTokens)) {
      ...
      tokenCounts[mint] = Math.floor(Math.random() * allTransactions.length * 0.3);
    }
